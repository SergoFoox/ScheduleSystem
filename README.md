...
src/main/java/com/sergofoox
  config/                 конфігурація Spring, Security, Solver
  domain/
...
Схема таблиць ініціалізується SQL-скриптами з `src/main/resources/db/migration`. У поточній конфігурації Flyway підключений як залежність, але міграції виконуються через `spring.sql.init`.
...
Актуальна production-збірка перевіряється командою:

```powershell
.\mvnw.cmd "-Dmaven.test.skip=true" package
```

Повний запуск `.\mvnw.cmd test` потребує актуалізації одного legacy-тесту генератора (`ScheduleServiceGenerationTest`), який посилається на старі внутрішні методи `ScheduleService`. Основний застосунок при цьому компілюється та збирається.

## Поточні технічні нотатки

- Проект працює у демонстраційному режимі: основні Hilla endpoints відкриті через `@AnonymousAllowed`.
- У `SecurityConfig` підготовлено Spring Security, але рольова авторизація не є фінальним production-механізмом.
- README описує поточний стан системи після переходу від чернеток/аналітики до практичного сценарію генерації, збереження та експорту розкладу.
